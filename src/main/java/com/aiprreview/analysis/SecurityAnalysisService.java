...
public class SecurityAnalysisService {
	...
	@Value("classpath:prompts/security-vulnerability-template.txt")
	private Resource securityPromptTemplate;

	private final AiProviderRouter aiProviderRouter;
	private final SecurityAnalysisResultParser resultParser;

	public SecurityAnalysisResult analyze(PullRequestWithFilesResponse pr, String preferredProvider) {
		AiProvider provider = aiProviderRouter.resolveProvider(preferredProvider);
		log.info("Running security analysis on PR #{} repo={} provider={}",
		...
		log.info("Security analysis AI response received provider={} contentLength={}",
				response.provider(), response.content() != null ? response.content().length() : 0);

		SecurityAnalysisResult result = resultParser.parse(response.content(), response.provider());
		log.info("Security analysis parsed: vulnerabilityCount={} riskLevel={} confidence={}",
				result.getVulnerabilityCount(), result.getRiskLevel(), result.getConfidence());
	...
	private String loadTemplate() {
		try {
			return new String(securityPromptTemplate.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
		} catch (IOException ex) {
			log.warn("Could not load security-vulnerability-template.txt, using minimal fallback. reason={}", ex.getMessage());
			return "Analyze the following PR diff for security vulnerabilities and return JSON with fields: " +
					"summary, vulnerabilityCount, riskLevel, vulnerabilities[], confidence.\n\nDiff:\n{{diffInput}}";
		}
	}
