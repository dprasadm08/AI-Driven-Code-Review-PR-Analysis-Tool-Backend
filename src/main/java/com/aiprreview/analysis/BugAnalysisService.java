...
public class BugAnalysisService {
    ...
    @Value("classpath:prompts/bug-detection-template.txt")
    private Resource bugPromptTemplate;

    private final AiProviderRouter aiProviderRouter;
    private final BugAnalysisResultParser resultParser;

    public BugAnalysisResult analyze(PullRequestWithFilesResponse pr, String preferredProvider) {
        AiProvider provider = aiProviderRouter.resolveProvider(preferredProvider);
        log.info("Running bug analysis on PR #{} repo={} provider={}",
        ...
        log.info("Bug analysis AI response received provider={} contentLength={}",
                response.provider(), response.content() != null ? response.content().length() : 0);

        BugAnalysisResult result = resultParser.parse(response.content(), response.provider());
        log.info("Bug analysis parsed: bugCount={} riskLevel={} confidence={}",
                result.getBugCount(), result.getRiskLevel(), result.getConfidence());
    ...
    private String loadTemplate() {
        try {
            return new String(bugPromptTemplate.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
        } catch (IOException ex) {
            log.warn("Could not load bug-detection-template.txt, using minimal fallback. reason={}", ex.getMessage());
            return "Analyse the following PR diff for bugs and return JSON with fields: " +
                    "summary, bugCount, riskLevel, bugs[], confidence.\n\nDiff:\n{{diffInput}}";
        }
    }
