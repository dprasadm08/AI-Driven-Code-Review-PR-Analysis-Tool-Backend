...
public class TestCaseAnalysisService {

	@Value("classpath:prompts/test-case-analysis.txt")
	private Resource testCasePromptTemplate;

	private final AiProviderRouter aiProviderRouter;
	private final TestCaseAnalysisResultParser resultParser;

	public TestCaseAnalysisResult analyze(PullRequestWithFilesResponse pr, String preferredProvider) {
		AiProvider provider = aiProviderRouter.resolveProvider(preferredProvider);
		log.info("Running test-case analysis on PR #{} repo={} provider={}",
		...
		log.info("Test-case analysis AI response received provider={} contentLength={}",
				response.provider(), response.content() != null ? response.content().length() : 0);

		TestCaseAnalysisResult result = resultParser.parse(response.content(), response.provider());
		log.info("Test-case analysis parsed: missingTestCount={} coverageRiskLevel={} confidence={}",
				result.getMissingTestCount(), result.getCoverageRiskLevel(), result.getConfidence());
	...
	private String loadTemplate() {
		try {
			return new String(testCasePromptTemplate.getInputStream().readAllBytes(), StandardCharsets.UTF_8);
		} catch (IOException ex) {
			log.warn("Could not load test-case-analysis.txt, using minimal fallback. reason={}", ex.getMessage());
			return "Analyze the following PR diff for missing test cases and return JSON with fields: " +
					"summary, missingTestCount, coverageRiskLevel, testFindings[], confidence.\\n\\nDiff:\\n{{diffInput}}";
		}
	}
